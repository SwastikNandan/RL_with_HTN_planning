...
# Adding sphere in clock_back_call_back function:
	...
	def delete_spheres(self):
		...
		kl = self.key_list
		for elem in kl:
			if np.linalg.norm(elem - self.uav_loc0) <= 0.5:
				#print("\n")
		...
		i = left_1
		for elem in kl:
			if np.linalg.norm(elem - self.uav_loc1) <= 0.5:
				#print("\n")
				#print("The location of the sphere is:", elem)
	...
	def add_spheres(self):
		...
		if len(self.deleted_queue) > 0:
			...
			if self.sim_time >= last_time + 3 and (np.linalg.norm(elem_cood - self.uav_loc0) >= 1 or np.linalg.norm(elem_cood - self.uav_loc1) >= 1):
			...
			elif self.sim_time >= last_time + 3 and (np.linalg.norm(elem_cood - self.uav_loc0) < 1 or np.linalg.norm(elem_cood - self.uav_loc1) < 1):
	...
	def execute_add(self, sphere_name, sphere_coordinates):
		...
		try: 
			...
			self.popped_bubble_list[sphere_index - 1] = 10
	...
	def return_state_uav0(self):
		...
		for cood in uav0_location_list:
			location_list.append(round(cood))
	...
	def return_state_uav1(self):
		...
		for cood in uav1_location_list:
			location_list.append(round(cood))
		return_list = location_list + self.popped_bubble_list
		array_to_publish = Float32MultiArray(data=return_list)

...
class OffboardControl:
    ...
    def __init__(self):
        ...
        self.hover_loc2 = [-6 , 1, 3, 0, 0, 0, 0]
        self.mode2 = "HOVER"
        self.direction = None
        self.arm2 = True
	self.number_of_explored_state = []
	self.list_of_cumulative_reward = []
	self.list_of_time_stamp = []
        self.dist_threshold = 0.1
        self.reward_list = []
        self.keypoints = []

        self.action_list = ['go-forward','go-right','go-back','go-left']
        self.gamma = 0.9
        self.alpha = 0.3
    ...
    def go_straight(self, step, state):
        ...
        while not rospy.is_shutdown():
            ...
            pose_pub.publish(self.des_pose)
            if self.completion_tag == "done":
		#print("The position of the UAV is:", self.curr_pose2)
                #break
                rate.sleep()
                return ['successful', self.statel]

    def go_back(self, step, state):
    ...
    def Q_learning(self):
        ...
        reward_prev = 0
        reward_cum = 0
        while(rospy.get_rostime().secs < self.initial_time + 1200) and self.mode2 == "Q-learning" and not rospy.is_shutdown():
	    ...
	    prunned_action_list = self.prunning_action_list(self.state)
            ...
            success, next_state = self.execute_action(action_to_perform, self.state)      # state will get updated through the variable next_state each time an action takes place.
	    ...
	    self.number_of_explored_state.append(len(self.state_dict))
	    self.list_of_cumulative_reward.append(reward_prev)
	    self.list_of_time_stamp.append(time_elapsed)
            self.state = next_state
            reward_prev = reward_cum
	    #print("The state dictionary is:", self.state_dict)
            step =  step + 1
	file = open('generated.cvs', 'w+')
	with file:
	    write = csv.writer(file)
	    print(self.number_of_explored_state)
	    print(self.list_of_cumulative_reward)
	    print(self.list_of_time_stamp)
	    write.writerows([self.number_of_explored_state, self.list_of_cumulative_reward, self.list_of_time_stamp])

    def execute_action(self, action_to_perform, state):
        if action_to_perform == 'go-forward':
	    print("Going forward")
            success_flag, next_state = self.go_straight(2, state)

        if action_to_perform == 'go-up':
	    print("Going up")
            success_flag, next_state = self.go_up(2, state)

        if action_to_perform == 'go-right':
	    print("Going right")
            success_flag, next_state = self.go_right(2, state)

        if action_to_perform == 'go-back':
	    print("Going back")
            success_flag, next_state = self.go_back(2, state)

        if action_to_perform == 'go-down':
	    print("Going down")
            success_flag, next_state = self.go_down(2, state)

        if action_to_perform == 'go-left':
	    print("Going left")
            success_flag, next_state = self.go_left(2, state)
